...
# --- DATE CHECK FOR STEP CLEANUP ---
# If the step check folder exists but is from a previous day, clear it to start fresh
if (Test-Path $StateDir) {
    $folderDate = (Get-Item $StateDir).LastWriteTime.Date
    $today = Get-Date -Format "yyyy-MM-dd"
    if ($folderDate.ToString("yyyy-MM-dd") -ne $today) {
        Write-Log "   -> Step check folder is from a previous date. Clearing progress..." "DarkOrange"
...
# COMMENTED OUT RUN ALL STEPS PER REQUEST
...
# Render the GUI
$MainForm.ShowDialog() | Out-Null# --- Pre-flight Check: Ensure Admin Rights ---
if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    Write-Host "[-] This script requires Administrator privileges. Please run PowerShell as Admin." -ForegroundColor Red
    exit
}
...
function Run-Step7 {
    ...
    try {
        ...
        # Optimized cleanup: process each path only once
        foreach ($path in $junkPaths) {
            if (Test-Path $path) {
                # Get all items and remove them in one pipeline to reduce overhead
