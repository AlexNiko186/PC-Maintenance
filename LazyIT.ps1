...
function Save-StepState ([int]$StepNum) {
    ...
    Set-Content -Path $StateFile -Value $StepNum -Force -ErrorAction SilentlyContinue
}

function Get-StepState {
    if (Test-Path $StateFile) { return [int](Get-Content $StateFile -ErrorAction SilentlyContinue) }
    return 0
}
...
function Run-Step1 {
    ...
    $btnApply.Add_Click({
        ...
        Save-StepState 1
        $script:btnStep1; $script:btnStep1.Text = "✅ 1. Configure Windows Settings"; $script:btnStep1
    })
...
function Run-Step2 {
    ...
    try {
        ...
        Save-StepState 2
        $script:btnStep2; $script:btnStep2.Text = "✅ 2. Run Windows Updates"; $script:btnStep2
        Show-RestartPrompt
...
function Run-Step3 {
    ...
    $btnUpdate.Add_Click({
        ...
        Save-StepState 3
        $script:btnStep3.Enabled = $false; $script:btnStep3.Text = "✅ 3. Update Apps (Winget)"; $script:btnStep3.BackColor = [System.Drawing.Color]::LightGray
        Show-RestartPrompt
    })
        ...
        try {
            if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
                ...
                if ($cleanOut -match "No installed package found matching input criteria" -or $cleanOut -match "No available upgrades") {
                    ...
                    Save-StepState 3
                    $script:btnStep3; $script:btnStep3.Text = "✅ 3. Update Apps (Winget)"; $script:btnStep3
                } else {
...
function Run-Step4 {
    ...
    try {
        ...
        if (Test-Path $ccleanerExe) {
            ...
            Save-StepState 4
            $script:btnStep4; $script:btnStep4.Text = "✅ 4. Launch CCleaner"; $script:btnStep4
            Show-RestartPrompt
...
function Run-Step5 {
    ...
    try {
        ...
        if (Test-Path $revoExe) { 
            ...
            Save-StepState 5
            $script:btnStep5; $script:btnStep5.Text = "✅ 5. Launch Revo Uninstaller"; $script:btnStep5
            Show-RestartPrompt
...
function Run-Step6 {
    ...
    try {
        ...
        Save-StepState 6
        $script:btnStep6; $script:btnStep6.Text = "✅ 6. Apply Clinic Optimizations"; $script:btnStep6
...
$resumeState = Get-StepState
if ($resumeState -gt 0) {
    Write-Log "   -> Resuming session. Marking completed steps..." "DarkOrange" -Bold
    if ($resumeState -ge 1) { $script:btnStep1.Text = "✅ 1. Configure Windows Settings" }
    if ($resumeState -ge 2) { $script:btnStep2.Text = "✅ 2. Run Windows Updates" }
    if ($resumeState -ge 3) { $script:btnStep3.Text = "✅ 3. Update Apps (Winget)" }
    if ($resumeState -ge 4) { $script:btnStep4.Text = "✅ 4. Launch CCleaner" }
    if ($resumeState -ge 5) { $script:btnStep5.Text = "✅ 5. Launch Revo Uninstaller" }
    if ($resumeState -ge 6) { $script:btnStep6.Text = "✅ 6. Apply Clinic Optimizations" }
}
